...
import type { Experience, ExperienceInput } from "@/types/auth";
...
export function ExperienceDialog({
  ...
  useEffect(() => {
    ...
    setCurrentlyWorking(Boolean(initial?.currentlyWorking));
  }, [open, initial]);

  const handleSave = async () => {
    if (!company.trim() || !jobTitle.trim() || !startDate) return;
    setSaving(true);
    try {
      await onSave({
        ...
        endDate: currentlyWorking ? undefined : fromMonthInput(endDate),
        currentlyWorking,
      });
      onOpenChange(false);
    } finally {
      ...
      <DialogContent className="gap-0 border-zinc-200 bg-white p-0 text-zinc-900 sm:max-w-lg dark:border-white/[0.08] dark:bg-[#161B22] dark:text-white">
        <DialogHeader className="space-y-2 px-6 pt-6 text-left">
          <DialogTitle className="flex items-center gap-2 text-base font-semibold text-zinc-900 dark:text-white">
            ...
            {initial ? "Update Work" : "Add Work"}
          </DialogTitle>
          <DialogDescription className="text-sm text-zinc-500 dark:text-zinc-400">
            Share your work experience on your profile.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 px-6 py-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-600 dark:text-zinc-300">Company</Label>
              <Input
                value={company}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-600 dark:text-zinc-300">Job Title</Label>
              <Input
                value={jobTitle}
